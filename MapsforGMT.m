...
nstart       = 8;                         % first output frame to process
nend         = 8;                         % last output frame to process
...
ndepth       = 95     % Number of the layer to map
...
plotany             = false;
projectT            = true;     save_projectT_data    = true;     % project temperature field and save it ?
...
projectDIV          = false;      save_projectDIV_data  = false;     % project divergence field and save it ?
   ...
   % NICO             [ierr]   = savecoolplots(save_str,fig_num,save_pdf,save_eps,save_jpg,save_png);
    %    close(h)
            ...
            if (save_projectDIV_data)
                [DAT] = savedata(DATA2D,thetad,phid);
                save_string = strcat(dir,fname,nfn,'_project_DIV.dat');
