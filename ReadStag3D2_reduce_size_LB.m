function [varargout] = ReadStag3D2(directory, fname_input, fname_number, Type,size_reduction,nr_save,nrstart)% Reads a STAG3D output file and transforms it into a MATLAB file
%
...
switch Type
    ...
    case 'heat flux'
        ...
        scalardata  = true;
    case 'divergence'
        fname       = [fname_input,'_div',number_string];
    scalardata  = true;
    case 'vorticity'
        fname       = [fname_input,'_vor',number_string];
    scalardata  = true;    
    otherwise
...
decrease_filesize = size_reduction;

nr_saveX = nr_save(1);   %saves every 'nr_saveX' number in x-direction
nr_saveY = nr_save(2);   %                              in y-direction
nr_saveZ = nr_save(3);   %                              in z-direction
    ...
    if nblocks==1
        ...
        switch Type
            ...
            case 'residue'
                ...
                dummy = P_3D(1:nr_saveX:numx,1:nr_saveY:numy,1:nr_saveZ:numz); P_3D = dummy;
            case 'topography'
               dummy = DATA_3D(1:nr_saveX:numx,1:nr_saveY:numy,1:1:numz,:); DATA_3D = dummy;
            case 'topography self-grav'
               dummy = DATA_3D(1:nr_saveX:numx,1:nr_saveY:numy,1:1:numz,:); DATA_3D = dummy;
            case 'geoid'
               dummy = DATA_3D(1:nr_saveX:numx,1:nr_saveY:numy,1:1:numz,:); DATA_3D = dummy;
        ...
        % ying-yang grid
        numx = size(X_3D,1); numy = size(X_3D,2); numz = size(X_3D,3);
        dummy = X_3D(1:nr_saveX:numx,1:nr_saveY:numy,nrstart:nr_saveZ:numz,:); X_3D = dummy;
        dummy = Y_3D(1:nr_saveX:numx,1:nr_saveY:numy,nrstart:nr_saveZ:numz,:); Y_3D = dummy;
        dummy = Z_3D(1:nr_saveX:numx,1:nr_saveY:numy,nrstart:nr_saveZ:numz,:); Z_3D = dummy;
       
        switch Type
            case 'velocity'
                dummy = VX_3D(1:nr_saveX:numx,1:nr_saveY:numy,nrstart:nr_saveZ:numz,:); VX_3D = dummy;
                dummy = VY_3D(1:nr_saveX:numx,1:nr_saveY:numy,nrstart:nr_saveZ:numz,:); VY_3D = dummy;
                dummy = VZ_3D(1:nr_saveX:numx,1:nr_saveY:numy,nrstart:nr_saveZ:numz,:); VZ_3D = dummy;
                dummy = P_3D(1:nr_saveX:numx,1:nr_saveY:numy,nrstart:nr_saveZ:numz,:); P_3D = dummy;
            % For the cases that are only defined at surface and cmb do not
            % reduce file size in z/radial-direction
            case 'residue'
                dummy = VX_3D(1:nr_saveX:numx,1:nr_saveY:numy,nrstart:nr_saveZ:numz,:); VX_3D = dummy;
                dummy = VY_3D(1:nr_saveX:numx,1:nr_saveY:numy,nrstart:nr_saveZ:numz,:); VY_3D = dummy;
                dummy = VZ_3D(1:nr_saveX:numx,1:nr_saveY:numy,nrstart:nr_saveZ:numz,:); VZ_3D = dummy;
            ...
            case 'heat flux'
               dummy = DATA_3D(1:nr_saveX:numx,1:nr_saveY:numy,1:1:numz,:); DATA_3D = dummy;
            case 'temperature'
               dummy = DATA_3D(1:nr_saveX:numx,1:nr_saveY:numy,nrstart:nr_saveZ:numz,:); DATA_3D = dummy;
               display(['...saved every ',num2str(nr_saveX),'. x-grid point, ',num2str(nr_saveY),'. y-grid point, ',num2str(nr_saveZ),'. z-grid point'])
            otherwise
               dummy = DATA_3D(1:nr_saveX:numx,1:nr_saveY:numy,nrstart:nr_saveZ:numz,:); DATA_3D = dummy;
        end
    end
